This repository contains all the code and datasets associated with the dissertation thesis:

Analysis of European Inflation: CPI Forecasting and Its Relationship with the Producer Price Index (PPI)
Faculty of Administration and Business – University of Bucharest
Business Data Analysis (ADA) - Cybernetics and Statistics
Coordinator: Prof. Bogdan Oancea, PhD – bogdan.oancea@faa.unibuc.ro
Author: Sorin Tudor – sorin.tudor@s.unibuc.ro
Year: 2026


This project has no license applied at the moment.
All rights reserved until the public defense of the dissertation.
Please do not reuse or distribute without explicit permission.
